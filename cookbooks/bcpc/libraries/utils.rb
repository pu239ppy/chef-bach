...
def get_head_nodes
  ...
  #
    BACH::ClusterData.new(node).fetch_cluster_def.select { |hst| hst[:runlist].include? "role[BCPC-Hadoop-Head]" or hst[:runlist].include? "role[BCPC-Kafka-Head-Zookeeper]" }
end
